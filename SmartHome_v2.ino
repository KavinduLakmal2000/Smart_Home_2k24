...
void notifySdStatus(const String& message) {
  Blynk.virtualWrite(V2, message);
}

void notifySdEvent(const char* eventName, const char* message) {
  Blynk.logEvent(eventName, message);
}

void addError(const String& message) {
  SDCardManager::addError(errorBuffer, timestamp, message);
}
...
void renderConnectedDisplay() {
  DisplayManager::DisplayData displayData = getDisplayData();

  if (VoltSensor < 200) {
    DisplayManager::drawDisplay3(display, displayData, SCREEN_WIDTH, SCREEN_HEIGHT);
    display.display();
    return;
  }
  ...
  switch (currentDisplay) {
    case 0:
      DisplayManager::drawDisplay1(display, displayData);
      break;
    case 1:
      DisplayManager::drawDisplay2(display, displayData);
      break;
    case 2:
      DisplayManager::drawDisplay3(display, displayData, SCREEN_WIDTH, SCREEN_HEIGHT);
      break;
    case 3:
      DisplayManager::drawDisplay4(display, SCREEN_WIDTH, SCREEN_HEIGHT);
      break;
  }
...
ConnectionManager::ConnectionContext getConnectionContext() {
  return {
    display,
    rgbLed,
    ssid,
...
void setup() {
  ...
  // ------------------------------------------------------------------- SD card --------------------------------------------
  SDCardManager::setCallbacks(notifySdStatus, notifySdEvent);
  SDCardManager::begin(spi, SD_SCK, SD_MISO, SD_MOSI, SD_CS);

  SDCardManager::updateAboutFile(SoftVer);
  delay(500);
  timeDateUpdate();
  SDCardManager::logFirmwareVersion(timestamp, SoftVer, whatsNew);
  delay(100);
  SDCardManager::checkCardSpace();
  delay(100);
  SDCardManager::sendAboutInfo();
  delay(200);
  if (SDCardManager::getSetting("Xmode") == HIGH) {
  ...
  if (SDCardManager::getSetting("AutoLight") == HIGH) {
  ...
  if (SDCardManager::getSetting("m_AutoLight") == HIGH) {
...
void loop() {  //------------------------------------------------------------------------------------------------ loop start ------------------------------------------------------------
  ...
  SerialReaderManager::SerialReaderContext serialReaderContext = getSerialReaderContext();
  SerialReaderManager::run(serialReaderContext);   // read promini serial data
  XModeManager::XModeContext xModeContext = getXModeContext();
  XModeManager::run(xModeContext);  // the room x mode
  SecurityModeManager::SecurityModeContext securityModeContext = getSecurityModeContext();
  SecurityModeManager::run(securityModeContext);  // activate with button, all pir's working at same time, alarm lock off
  TemperatureReaderManager::TemperatureReaderContext temperatureReaderContext = getTemperatureReaderContext();
  TemperatureReaderManager::run(temperatureReaderContext);      // read all the temp sensors and filter output values
  AutoSecurityManager::run(getAutoSecurityContext());  // pir sensors at out side automatic work and notifications
  IRSwitchManager::IRSwitchContext irSwitchContext = getIRSwitchContext();
  IRSwitchManager::run(irSwitchContext);

  if (MiN != lastMinute) {  // write data in SD card every 1 Min
    lastMinute = MiN;
    SDCardManager::SensorLogData logData = {
      timestamp,
      VoltSensor,
      AmpSensor,
      batteryVoltage_main,
    ...
    SDCardManager::logData(logData, logMainVoltStatus);
    SDCardManager::flushErrors(errorBuffer, timestamp);
  }
  //--------------------------------------------------------------
  ...
  if (Local_autoLight) {
    AutoLightManager::AutoLightContext autoLightContext = getAutoLightContext();
    AutoLightManager::runAutoLight(autoLightContext);  // automatic turn on lights when house main power off at 6PM to 10PM
  }

  if (Local_m_autoLight) {
    AutoLightManager::AutoLightContext autoLightContext = getAutoLightContext();
    AutoLightManager::runMidnightAutoLights(autoLightContext);  // automatic turn on lights when house main power is off at 10PM to 5AM (only when inside pir detects someone)
  }

  if (safetyMode) {
    SafetySystemManager::SafetySystemContext safetySystemContext = getSafetySystemContext();
    SafetySystemManager::run(safetySystemContext);  // checking all the temp sensors and voltage sensors if any problems send notification and cmd lines
    if (flag8) {
  ...
  if (testV2cmd) {  // for testing values cmd printing command for active "test"
    ...
    AutoLightManager::AutoLightContext autoLightContext = getAutoLightContext();
    AutoLightManager::runMidnightAutoLights(autoLightContext);
  }
  ...
  ConnectionManager::ConnectionContext connectionContext = getConnectionContext();
  ConnectionManager::run(connectionContext);
...
BLYNK_WRITE(V2) {  // ----------------------------------------------------------------------------------------- cmd command reading -----------------------
  CommandManager::CommandContext commandContext = {
    cmd,
    pcf1,
    VoltSensor,
    pirCounter,
  ...
  CommandManager::handleCommand(commandContext, param.asStr());
}
